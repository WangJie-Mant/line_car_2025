...
//      receiving_process();//解析野火上位机通信协议,调试电机位置速度串级PID时用,不用时注释  	
      ...
      else if(g_mode == 1)//角度环直行
      {
      
				  HAL_Delay(1000);//给出时间拿开手，防止干扰车子启动
					car_go(100,0);
      }
      else if(g_mode == 2)//环场地一圈
      {
					HAL_Delay(1000);
					car_go(100,0);					
					car_go_line(126);
      ...
      else if(g_mode == 3)//八字弯一圈
      {
					HAL_Delay(1000);
					target_degree=-51.34;
					car_spin_degree(target_degree);
					car_go(128,-51.34);
					car_go_line(126);
					target_degree=-128.66;
					car_spin_degree(target_degree);
					car_go(128,-128.66);
					car_go_line(126);
           ...
           if (key)
            {
                switch (key)
                {
                    case KEY0_PRES:
                        ...
                        target_degree = target_degree+90;
                        if(target_degree>360)target_degree = 0;
                        set_pid_target(&g_pid_turn_angle,target_degree);
                        break;
                    case KEY1_PRES:
                        HAL_Delay(500);//给出时间拿开手，防止干扰车子启动
                        target_degree = target_degree+180;
                        if(target_degree>360)target_degree = 0;
                        set_pid_target(&g_pid_turn_angle,target_degree);
                        break;
                }
