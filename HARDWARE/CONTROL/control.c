...
float g_ftarget_journey = 0; // 存放小车左右轮走的路程,单位cm，需要在下一阶段任务中设置(car_go_line),用于判断车子到达目标位置
...
uint8_t g_Gostraight_Angle_Flag = 0; // 依靠角度环来走直线
    ...
    if (htim == &htim3) // 20ms定时器中断进行PID计算
    {
        ...
        if (g_Gostraight_Angle_Flag == 1)
        {
            ...
            if (g_is_motor1_en == 1 || g_is_motor2_en == 1) // 电机在使能状态下才进行控制处理
            {
                ...
                // 这个是灰度传感器的巡线补偿
                angle_compensation = straight_angle_pid_calculate();
                g_line_outval = angle_compensation;
...
/*这里加将位置环的输入直接编程距离(cm)的函数 */ // 所有巡线都用这个
...
void car_go(int32_t distance_cm, float target_angle) // 直走函数     //连续两次的直行好像会让巡线补偿失效
{
    float target_pluse; // 目标距离对应的脉冲值
    set_straight_target_angle(target_angle);
    /* 设置对应标志位 */
    g_Line_Flag = 0;
    g_Stop_Flag = 0;
    g_Gostraight_Angle_Flag = 1;
    g_Spin_Start_Flag = 0;
    g_Spin_Succeed_Flag = 0;
    g_Angle_Flag = 0;
    /* 清除上次走的路程记录 */
...
float location1_pid_control(void)
{
    ...
    actual_location = g_sigma_motor1pluse; // 1圈 = 1040个脉冲 = 20*13*4  //这里位置用圈数代替。
...
float speed1_pid_control(void)
{
    ...
    单位时间内脉冲/(编码器倍频后的总分 辨率*电机减速比*PID控制周期)-----每ms转的圈数
