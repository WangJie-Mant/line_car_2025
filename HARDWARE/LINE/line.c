...
float line_pid_realize(int err)
{
    ...
    // 积分限幅，防止积分过大
    if (line_pid_integral > 5000)
        line_pid_integral = 5000;
    if (line_pid_integral < -5000)
        line_pid_integral = -5000;
    return out;
}
...
int32_t line_err(void)
{
    ...
    if (weight == 0)
    {
        g_Line_On_Flag = 0; // 没有检测到线
        return 0;           // 无线，保持直行
    }
    else
    {
        g_Line_On_Flag = 1;                    // 检测到线
...
uint8_t Is_Off_Line(void)
{
    if (g_Line_On_Flag == 0)
    {
        return 0;
    }
    static uint8_t off_line_count = 0;
    ...
    if (line_err() == 0)
    {
        ...
        {
            // 20ms一个定时器中断，只要连续1秒没有检测到线，就认为掉线了
            off_line_count = 0;
            return 1;
        }
    }
